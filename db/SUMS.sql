...
-- Chèn thêm 6 tin tức cho đủ bộ 10 bài
INSERT INTO News (Title, Content, ImageUrl, CreatedByUserId)
VALUES 
...
-- cập nhật các căn có contract và còn hiệu lực là 1. 
UPDATE a
SET a.Status = 1
FROM Apartments a
...
SELECT 
    r.Id,            -- Lấy ResidentId (1, 2, 3, 4)
    c.ApartmentId,   -- Lấy Mã Căn (1, 2, 3, 4, 5, 6)
    c.StartDate,     -- Ngày bắt đầu ở giống ngày hiệu lực Contract
    NULL             -- MoveOut để NULL vì họ vừa mới mua nhà xong
FROM [SmartCity_DB].[dbo].[Residents] r
JOIN [SmartCity_DB].[dbo].[Contracts] c ON r.AccountId = c.AccountId
WHERE c.Status = 1; -- Chỉ lấy những hợp đồng đang "xanh"
GO
